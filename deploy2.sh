...
# Check for lsof (for port checks)
if ! command -v lsof &> /dev/null; then
echo "❌ ERROR: The 'lsof' command is not installed."
echo "This is required for checking if ports are in use."
echo "  - On Ubuntu/Debian: sudo apt-get install lsof"
echo "  - On RHEL/CentOS: sudo yum install lsof"
echo "  - On macOS: lsof is usually pre-installed"
exit 1
fi
...
JWT_SECRET=(generate_jwt_secret)ENCRYPTION_KEY=(generate_key)
...
JWT_SECRET_KEY=JWT_SECRETCREDENTIAL_ENCRYPTION_KEY={ENCRYPTION_KEY}
...
Step 8: Start database first with retry logic
The database must be running before the backend tries to connect to it.
echo "Starting the database service with retry logic..."
DB_PORT=5433
check_port_availability() {
local port=$1
local max_attempts=10
local attempt=1
echo "Checking if port $port is available on the host..."
while [ $attempt -le $max_attempts ]; do
if ! lsof -i :$port >/dev/null 2>&1; then
echo "✅ Port $port is available on the host."
return 0
fi
echo "Waiting for host port $port to be released... (attempt $attempt/max_attempts)"sleep2attempt=((attempt + 1))
done
echo "❌ ERROR: Port $port is still in use on the host after multiple attempts. Cannot proceed."
exit 1
}
check_port_availability $DB_PORT

Function to start the database container with retries for the bind error
start_db_with_retry() {
local max_retries=10
local retry_delay=5
for i in $(seq 1 $max_retries); do
echo "Attempting to start database container (attempt $i/max_retries)..."#The‘∣∣true‘isusedtopreventthescriptfromexitingimmediatelyonfailure#Theoutputiscapturedtocheckforthespecificerrormessageoutput=($COMPOSE_CMD up -d db 2>&1)
if echo "$output" | grep -q "Error starting userland proxy"; then
echo "⚠️  WARNING: Encountered 'port already in use' error from Docker. Retrying in $retry_delay seconds..."
...
echo "❌ ERROR: Database startup failed with an unexpected error."
echo "Docker output: $output"
exit 1
fi
fi
done
echo "❌ ERROR: Failed to start the database container after $max_retries attempts."
exit 1
}
start_db_with_retry
echo ""
...
echo "Waiting for database... (attempt $db_attempt/max_db_attempts)"sleep2db_attempt=((db_attempt + 1))
done
...
while [ $attempt -le max_attempts];dohealth_response=(curl -s http://localhost:8001/health 2>/dev/null)
if [ ! -z "$health_response" ]; then
echo "✅ Backend health check passed."
echo "    Health status: $health_response"
break
fi
echo "Waiting for backend... (attempt $attempt/max_attempts)"sleep2attempt=((attempt + 1))
done
...
echo "Waiting for frontend... (attempt $frontend_attempt/max_frontend_attempts)"sleep2frontend_attempt=((frontend_attempt + 1))
done
...
echo "  3. Check Docker logs: COMPOSE_CMDlogsbackend"exit1fi#CheckfrontendNode.jsdependenciesecho"Checkingfrontenddependencies..."iffrontend_deps_check=($COMPOSE_CMD exec -T frontend node -e "
const fs = require('fs');
