# Location: mixview/backend/models.py
# Description: Database models (renamed from backend.py to avoid conflicts)
...
class Filter(Base):
    ...
    filter_type = Column(String)  # e.g., "exclude_genre", "min_similarity"
    value = Column(String)
...
class UserServiceCredential(Base):
    ...
    service_name = Column(String, nullable=False)  # 'spotify', 'lastfm', 'discogs'
    credential_type = Column(String, nullable=False)  # 'api_key', 'oauth_token', 'client_credentials'
    encrypted_data = Column(Text, nullable=False)  # JSON of encrypted credentials
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    ...
    # Unique constraint per user per service
    __table_args__ = (
